...
func TestAppendArgs(t *testing.T) {
	ok := &config{FooBar: "123", Baz: "456", AFrom: "ok", B: "b", Nested: Nested1{D: "d"}, NestedP: &Nested2{E: "e"}}
	...
	t.Run("ok/unknown", func(t *testing.T) {
		check(t, ok, getValue, "unknown", []string{"--foobar=123", "--baz=456", "--afrom=OK", "--b=b", "--bool1=false", "--d=d", "--e=e"}, nil)
	})

	t.Run("ok/flag1", func(t *testing.T) {
		check(t, ok, getValue, "flag1", []string{"--foo-bar=123", "--baz=456", "--a=OK", "--bool1=false", "--d=d", "--e=e"}, nil)
	})
